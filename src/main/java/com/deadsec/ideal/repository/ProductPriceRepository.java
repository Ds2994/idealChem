...
public interface ProductPriceRepository extends JpaRepository<ProductPrice, Integer>{
	...
	@Query(value = "SELECT pp.id, pp.price, pk.packing_size FROM productprice pp "
			...
			+ "ORDER BY pp.price",
	nativeQuery = true)
	public List<Object[]> getPricePackListByProductCompany(@Param("pId") int productId, @Param("cId") int companyId);
	...
	//Fetch the company Name and Id by product_Id
	@Query(value = "SELECT DISTINCT(pp.company_id), c.company_name FROM productprice pp "
			+ "JOIN company c ON c.id = pp.company_id "
			+ "WHERE pp.product_id = :pId ",
	nativeQuery = true)
	public List<Object[]> getCompanyNameByProductId(@Param("pId") int productId);
}
